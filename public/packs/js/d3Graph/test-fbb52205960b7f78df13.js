...
d3.json("/characters/all/d3json#", function (error, graph) {
  ...
  var node = svg.append("g").attr("class", "nodes").selectAll("circle").data(graph.nodes).enter().append("circle").attr("r", 6).on("dblclick", dblclick).on("hover", hello).call(d3.drag().on("start", dragstarted).on("drag", dragged).on("end", dragended));
...
/******/ });
//# sourceMappingURL=test-fbb52205960b7f78df13.js.map